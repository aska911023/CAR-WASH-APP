...
const STATUS_MAP: Record<BookingStatus, { label: string; cls: string }> = {
    ...
    confirmed: { label: '已確認', cls: 'bg-gray-100 text-gray-700 border-gray-300' },
    in_progress: { label: '服務中', cls: 'bg-gray-900 text-white border-gray-900' },
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-gray-900 pb-28 font-sans">
            ...
            <div className="px-5 pt-10 pb-0 sticky top-0 z-30 bg-gray-50">
                <h1 className="text-[22px] font-black tracking-tight text-gray-900 mb-1">我的預約</h1>
                <p className="text-[12px] text-gray-400 font-semibold mb-4">查看預約進度與歷史紀錄</p>
                <div className="flex border-b border-gray-200">
                    {(['upcoming', 'history'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={`flex-1 py-2.5 text-[13px] font-black border-b-2 transition-all ${activeTab === tab ? 'text-gray-900 border-gray-900' : 'text-gray-400 border-transparent'}`}>
            ...
            <div className="px-4 pt-4 space-y-3">
                {shown.map(booking => {
                    ...
                    return (
                        <div key={booking.id} className="bg-white rounded-[20px] border border-gray-100 shadow-card overflow-hidden">
                            ...
                            <div className="p-4">
                                <div className="flex gap-4 text-[12px] text-gray-500 font-semibold mb-3">
                                ...
                                {booking.status === 'in_progress' && (
                                    <div className="mb-4 bg-gray-50 border border-gray-200 rounded-[14px] p-3">
                                        <p className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-3">服務進度</p>
                                                ...
                                                return (
                                                    <div key={idx} className="flex items-center gap-3">
                                                        {done
                                                            ...
                                                            : current
                                                                ? <div className="w-[18px] h-[18px] rounded-full border-2 border-gray-900 bg-gray-100 flex items-center justify-center shrink-0"><div className="w-2 h-2 bg-gray-900 rounded-full animate-pulse" /></div>
                                                                : <Circle size={18} className="text-gray-200 shrink-0" />
                                                        }
                                                        <span className={`text-[12px] font-bold ${done ? 'text-green-700 line-through' : current ? 'text-gray-900 font-black' : 'text-gray-500'}`}>
                                                            {step.icon} {step.label}
                                                        </span>
                                                        {current && <span className="ml-auto text-[10px] bg-gray-200 text-gray-700 px-2 py-0.5 rounded-full font-black animate-pulse">進行中</span>}
                                                    </div>
                                                );
                                            })}
                                        </div>
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-between py-2 text-[12px] text-gray-400 font-bold border-t border-gray-50 mt-1"
                                >
                                ...
                                {isOpen && (
                                    <div className="mt-2 space-y-3 animate-in">
                                        ...
                                        <div className="flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2.5">
                                            <span className="text-[12px] font-bold text-gray-600">服務費用</span>
                                            <span className="text-[16px] font-black text-gray-900">{booking.price.toLocaleString()} P</span>
                                        </div>
                                        ...
                                        <div className="flex gap-2">
                                            {isActive && !etaSent.includes(booking.id) && (
                                                <button
                                                    ...
                                                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-gray-900 text-white rounded-xl text-[12px] font-black shadow-lg active:scale-95 transition-all"
                                                >
                                        ...
                                        {booking.status === 'in_progress' && (
                                            <div>
                                                <p className="text-[11px] font-black text-gray-400 mb-2 uppercase tracking-widest">門店施工照片</p>
                                                <div className="grid grid-cols-3 gap-2">
                                                    <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=200&auto=format&fit=crop" className="w-full h-20 object-cover rounded-xl border border-gray-100" alt="" />
                                                    <div className="w-full h-20 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex flex-col items-center justify-center text-gray-400 cursor-pointer">
                                                        <Camera size={16} />
