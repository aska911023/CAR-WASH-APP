...
// These must match the labels in Home.tsx SERVICES
...
export default function ShopsList() {
    ...
    // temp state inside filter sheet
    ...
    const filteredShops = MOCK_SHOPS
        ...
        .sort((a, b) => {
            ...
            return parseFloat(a.distance) - parseFloat(b.distance); // 距離最近
        });

    const activeCount = activeServices.length + (sortBy !== '距離最近' ? 1 : 0);

    return (
        <div className="min-h-screen text-neutral-900 bg-[#f0f4ff] pb-28 relative">

            {/* ── Header ─────────────────────────────── */}
            <div className="px-5 pt-10 pb-3 sticky top-0 z-30 bg-[#f0f4ff]">
                <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => navigate(-1)}
                            className="w-8 h-8 flex items-center justify-center rounded-full bg-white shadow-sm border border-neutral-200 text-neutral-600 active:scale-95 transition-all shrink-0"
                        >
                            <ArrowLeft size={16} />
                        </button>
                        <div>
                            <h1 className="text-[22px] font-black tracking-tight" style={{ color: '#1e3a8a' }}>尋找洗車店家</h1>
                            <p className="text-[12px] font-semibold mt-0.5" style={{ color: '#3b82f6' }}>
                    ...
                    <button
                        onClick={openFilter}
                        className="flex items-center gap-1.5 px-3 py-2 bg-white shadow-card border rounded-xl active:scale-95 transition-all text-[13px] font-bold relative"
                        style={{ borderColor: activeCount > 0 ? '#2563eb' : '#e0e7ff', color: activeCount > 0 ? '#2563eb' : '#374151' }}
                    >
                        <SlidersHorizontal size={15} /> 篩選
                        {activeCount > 0 && (
                            <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full text-white text-[9px] font-black flex items-center justify-center" style={{ background: '#2563eb' }}>
                                {activeCount}
                            </span>
                        )}
                    </button>
                ...
                {/* Active service tag chips */}
                {activeServices.length > 0 && (
                    <div className="flex gap-1.5 overflow-x-auto nice-scrollbar pb-1">
                        {activeServices.map(s => (
                            <button key={s}
                                onClick={() => setActiveServices(prev => prev.filter(x => x !== s))}
                                className="shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black text-white"
                                style={{ background: '#2563eb' }}>
                                {s} <X size={10} />
                            </button>
                        ))}
                        <button onClick={() => setActiveServices([])} className="shrink-0 text-[11px] font-bold text-neutral-400 px-1">
                            清除全部
                        </button>
                    </div>
                )}
                ...
                <div className="px-5 flex flex-col gap-3">
                    {filteredShops.length === 0 ? (
                        <div className="flex flex-col items-center py-16 text-neutral-400">
                            ...
                            <button onClick={() => setActiveServices([])} className="mt-4 px-4 py-2 rounded-xl font-black text-[13px] text-white" style={{ background: '#2563eb' }}>清除篩選</button>
                        </div>
                    ) : (
                        filteredShops.map((shop) => (
                            <Link key={shop.id} to={`/shops/${shop.id}`}
                                className="flex gap-3 bg-white p-3 rounded-[20px] shadow-card border border-brand-50 active:scale-[0.98] transition-all">
                                ...
                                <div className="flex-1 flex flex-col justify-between py-0.5 pr-1 min-w-0">
                                    <div>
                                        <h3 className="text-[15px] font-black text-neutral-900 leading-tight truncate mb-0.5">{shop.name}</h3>
                                        <div className="flex items-center gap-1.5 text-[11px] text-neutral-400 font-semibold mb-2">
                                            <MapPin size={11} style={{ color: '#3b82f6' }} /> {shop.distance} · {shop.location}
                                        </div>
                                        <div className="flex flex-wrap gap-1.5">
                                            {shop.services.map((s) => (
                                                <span key={s}
                                                    className="px-2 py-0.5 text-[10px] font-bold rounded-md border transition-all"
                                                    style={activeServices.includes(s)
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-1 text-[13px] font-black text-neutral-800">
                                            <Star size={13} className="text-amber-400 fill-amber-400" /> {shop.rating}
                                            <span className="text-[11px] font-medium text-neutral-400 ml-1">({shop.reviews})</span>
                                        </div>
                                        <span className="text-[12px] font-black text-white px-3 py-1.5 rounded-xl shadow-sm" style={{ background: '#2563eb' }}>預約</span>
                                    </div>
                                </div>
                            </Link>
                        ))
            ...
            {viewMode === 'map' && (
                <div className="px-5">
                    <div className="relative w-full h-[460px] rounded-[24px] overflow-hidden shadow-[0_8px_30px_rgba(37,99,235,0.15)] border border-brand-100">
                        ...
                        {filteredShops.map((shop, i) => (
                            <button key={shop.id} onClick={() => setSelectedPin(selectedPin === i ? null : i)}
                                ...
                                <div className={`px-2.5 py-1 rounded-xl text-[11px] font-black shadow-lg border-2 transition-all ${selectedPin === i
                                    ? 'bg-brand-600 text-white border-white scale-110'
                                    : 'bg-white text-brand-700 border-brand-200'}`}>
                                    ⭐ {shop.rating}
                                </div>
                                <div className={`w-2 h-2 rounded-full ${selectedPin === i ? 'bg-brand-600' : 'bg-brand-400'}`} />
                            </button>
                        ))}

                        {selectedPin !== null && filteredShops[selectedPin] && (
                            <div className="absolute bottom-4 left-4 right-4 bg-white rounded-2xl p-4 shadow-[0_8px_30px_rgba(0,0,0,0.15)] border border-brand-100 animate-in">
                                <button onClick={() => setSelectedPin(null)} className="absolute top-3 right-3 text-neutral-400"><X size={16} /></button>
                                <div className="flex gap-3">
                                    <img src={filteredShops[selectedPin].image} alt="" className="w-14 h-14 rounded-xl object-cover" />
                                    <div className="flex-1 min-w-0">
                                        <h4 className="font-black text-[15px] text-neutral-900 truncate">{filteredShops[selectedPin].name}</h4>
                                        <p className="text-[11px] text-neutral-400 font-semibold">{filteredShops[selectedPin].distance} · {filteredShops[selectedPin].location}</p>
                                        <div className="flex items-center gap-1 mt-1">
                                            <Star size={12} className="fill-amber-400 text-amber-400" />
                                            <span className="text-[12px] font-black">{filteredShops[selectedPin].rating}</span>
                                        </div>
                                    </div>
                                    <Link to={`/shops/${filteredShops[selectedPin].id}`} className="self-center text-[12px] font-black text-white px-3 py-2 rounded-xl shadow-sm shrink-0" style={{ background: '#2563eb' }}>
                                        預約
                                    </Link>
                                </div>
                            </div>
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            <div className="w-4 h-4 bg-brand-600 rounded-full border-2 border-white shadow-lg" />
                            <div className="absolute inset-0 w-4 h-4 bg-brand-400 rounded-full animate-ping opacity-60" />
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex flex-col justify-end" style={{ background: 'rgba(0,0,0,0.5)' }}
                    ...
                    <div className="bg-white rounded-t-[28px] w-full max-w-[430px] mx-auto" style={{ maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-neutral-100 shrink-0">
                            <button onClick={resetFilter} className="text-[13px] font-black" style={{ color: '#ef4444' }}>重置</button>
                            <h2 className="text-[15px] font-black text-neutral-900 flex items-center gap-2"><Filter size={15} /> 篩選條件</h2>
                            <button onClick={() => setShowFilter(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-neutral-100 text-neutral-500">
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                            ...
                            {/* Sort */}
                            <div>
                                <p className="text-[11px] font-black text-neutral-400 uppercase tracking-widest mb-2.5">排序方式</p>
                                <div className="flex gap-2 flex-wrap">
                                    {SORT_OPTIONS.map(opt => (
                                        <button key={opt} onClick={() => setDraftSort(opt)}
                                            className="px-3.5 py-2 rounded-xl text-[12px] font-black border transition-all"
                                            style={draftSort === opt
                                                ? { background: '#2563eb', color: '#fff', borderColor: '#2563eb' }
                            ...
                            {/* Service Tags */}
                            <div>
                                <p className="text-[11px] font-black text-neutral-400 uppercase tracking-widest mb-2.5">服務類型（可多選）</p>
                                <div className="grid grid-cols-2 gap-2">
                                    {ALL_SERVICE_TAGS.map(tag => {
                                        const active = draftServices.includes(tag);
                                        return (
                                            <button key={tag} onClick={() => toggleDraftService(tag)}
                                                className="flex items-center gap-2 px-3.5 py-2.5 rounded-[14px] border text-[13px] font-bold transition-all"
                                                style={active
                                                    ? { background: '#eff6ff', color: '#1d4ed8', borderColor: '#2563eb' }
                                                    : { background: '#f8fafc', color: '#374151', borderColor: '#e2e8f0' }}>
                                                {active && <CheckCircle2 size={14} style={{ color: '#2563eb' }} />}
                                                {tag}
                                            </button>
                                        );
                                    })}
                        ...
                        {/* Apply button */}
                        <div className="px-5 pb-6 pt-3 border-t border-neutral-100 shrink-0">
                            <button onClick={applyFilter}
                                className="w-full py-3.5 rounded-2xl font-black text-[14px] text-white active:scale-95 transition-all"
