...
const MESSAGES: Record<string, { id: string; from: 'user' | 'shop'; text: string; time: string; read: boolean }[]> = {
    C1: [
        ...
        { id: 'm4', from: 'shop', text: '從大門進來後左轉，停車場在建築物後方，有我們的 WashHub 標誌 🅿️', time: '13:40', read: true },
        ...
        return (
            <div className="h-full flex flex-col bg-white">
                ...
                <div className="px-4 pt-12 pb-3 bg-white border-b border-gray-100 flex items-center gap-3 sticky top-0 z-30">
                    <button onClick={() => setActiveConv(null)} className="text-brand-600 font-black text-[13px]">←</button>
                        ...
                        <div key={msg.id} className={`flex ${msg.from === 'shop' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-[13px] font-medium leading-relaxed ${msg.from === 'shop'
                                ...
                                <div className={`flex items-center gap-1 mt-1 ${msg.from === 'shop' ? 'justify-end' : 'justify-start'}`}>
                                    <span className={`text-[10px] ${msg.from === 'shop' ? 'text-blue-200' : 'text-gray-400'}`}>{msg.time}</span>
                                    {msg.from === 'shop' && (msg.read ? <CheckCheck size={11} className="text-blue-200" /> : <Check size={11} className="text-blue-200" />)}
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="px-4 py-3 bg-white border-t border-gray-100 flex items-center gap-2">
                    <button className="w-9 h-9 flex items-center justify-center text-gray-400 active:text-brand-500">
                        <ImageIcon size={18} />
                    </button>
                    <button className="w-9 h-9 flex items-center justify-center text-gray-400 active:text-brand-500">
                        <Smile size={18} />
                    </button>
                    <div className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-2 focus-within:border-brand-400 transition-all">
                        <input
                            value={input}
                    ...
                    <button
                        onClick={sendMsg}
                        className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${input.trim() ? 'bg-brand-600 text-white shadow-blue-glow active:scale-95' : 'bg-gray-100 text-gray-400'}`}
                    >
                        <Send size={16} />
                    </button>
                </div>
            ...
            <div className="divide-y divide-gray-50">
                {CONVERSATIONS.filter(c => c.name.toLowerCase().includes(search.toLowerCase())).map(c => (
                    <button key={c.id} onClick={() => setActiveConv(c.id)} className="w-full text-left px-5 py-4 flex items-center gap-3 active:bg-gray-50 transition-colors">
                        ...
                        {c.unread > 0 && (
                            <span className="w-5 h-5 bg-brand-600 rounded-full flex items-center justify-center text-[10px] font-black text-white shrink-0">
                                {c.unread}
                            </span>
                        )}
                    </button>
